...
app.post('/auth/register', async (req,res) => {
  ...
  try {
    ...
    else { perfil=['normal','gestor'].includes(perfilSolicitado)?perfilSolicitado:'normal'; status='pendente' }
...
app.post('/auth/login', async (req,res) => {
  ...
  try {
    ...
    if (usuario.status==='inativo')  return res.status(403).json({ error:'Conta desativada.' })
...
app.patch('/admin/usuarios/:id', autenticar, apenasAdmin, async (req,res) => {
  const body = req.body; const updates = {}
  ...
  const { data, error } = await supabase.from('usuarios').update(updates).eq('id',req.params.id).select()
  if (error) return res.status(400).json({ error:error.message })
...
app.get('/medicos', autenticar, async (req,res) => {
  ...
  if (req.usuario.perfil !== 'admin' && req.usuario.clinica_id) {
    query = query.eq('clinica_id', req.usuario.clinica_id)
  }
...
app.post('/medicos', autenticar, async (req,res) => {
  ...
  const { data, error } = await supabase.from('medicos')
    .insert([{ nome, crm, especialidade, telefone, email, clinica_id, ativo:true, agenda: agenda||{} }]).select()
  if (error) return res.status(400).json({ error:error.message })
  res.status(201).json(data[0])
})
app.patch('/medicos/:id', autenticar, async (req,res) => {
  const updates = {}; const allowed = ['nome','crm','especialidade','telefone','email','ativo','agenda']
...
app.post('/consultas', autenticar, async (req,res) => {
  ...
  const { data, error } = await supabase.from('consultas')
    .insert([{ paciente_id, medico_id:medico_id||null, data_consulta, horario, motivo, observacoes, clinica_id }]).select()
  if (error) return res.status(400).json({ error:error.message })
  res.status(201).json(data[0])
})
...
app.post('/prontuarios', autenticar, async (req,res) => {
  ...
  const { data, error } = await supabase.from('prontuarios')
    .insert([{ paciente_id, descricao, data_registro, clinica_id:req.usuario.clinica_id }]).select()
  if (error) return res.status(400).json({ error:error.message })
  res.status(201).json(data[0])
})

// CLÍNICAS (geral autenticado)
