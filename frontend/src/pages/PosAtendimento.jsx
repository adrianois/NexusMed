...
// ─── Card de um paciente na fila ─────────────────────────────────────────────
function CardPaciente({ item, onSalvar, onFinalizar, navigate }) {
  ...
  return (
    <div style={{
      ...
      border: `1px solid ${
        finalizado ? 'rgba(74,222,128,0.2)' : 'rgba(255,255,255,0.07)'
      }`,
      borderRadius: '14px',
      marginBottom: '14px',
      overflow: 'hidden',
      ...
      >
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{
            ...
            color: finalizado ? '#4ade80' : '#e2e8f0',
          }}>
            {item.paciente?.nome || '—'}
          </div>
        ...
        <div style={{ padding: '0 18px 18px', borderTop: '1px solid rgba(255,255,255,0.05)' }}>
          ...
          {pront?.diagnostico && (
            <div style={{
              ...
              <span style={{ color: '#60a5fa', fontWeight: 700 }}>Diagnóstico: </span>
              {pront.diagnostico}
              {pront.retorno_dias > 0 && (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginTop: '14px' }}>
            ...
            <div>
              ...
              <select
                ...
                value={form.cobranca_status}
                onChange={e => set('cobranca_status', e.target.value)}
                disabled={finalizado}
              >
                {Object.entries(COB_CFG).map(([k, v]) => (
              ...
              <input
                ...
                value={form.cobranca_obs}
                onChange={e => set('cobranca_obs', e.target.value)}
                disabled={finalizado}
              />
            </div>
          </div>
          ...
          <div style={{ marginTop: '16px' }}>
            <div style={{
              ...
              textTransform: 'uppercase', letterSpacing: '0.08em',
              marginBottom: '8px',
            }}>📝 Observações de saída</div>
            <textarea
              className='form-textarea'
              rows={2}
              placeholder='Orientações dadas, intercorrências, observações gerais...'
              value={form.observacoes_saida}
              onChange={e => set('observacoes_saida', e.target.value)}
              disabled={finalizado}
...
export default function PosAtendimento() {
  ...
  const handleFinalizar = async (consulta_id, paciente_id, form, setSalvando) => {
    ...
    try {
      ...
      toast('Paciente finalizado com sucesso! ✅', 'success')
      carregar(data)
    } catch (err) {
      toast('Erro: ' + (err.response?.data?.error || err.message), 'error')
    } finally { setSalvando(false) }
  ...
  // Itens a exibir na lista principal
