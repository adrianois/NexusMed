...
export default function Retornos() {
  ...
  const carregar = async (params = {}) => {
    ...
    try {
      ...
      const { data } = await api.get(`/retornos?${q.toString()}`)
      setRetornos(data || [])
    } catch (err) {
  ...
  const abrirModal = (r) => setModal({
    paciente_id:   r.paciente_id,
    paciente_nome: r.paciente_nome,
    motivo:        r.diagnostico || r.motivo_original || 'Retorno',
    retorno_dias:  r.retorno_dias,
  })
  ...
  return (
    <PageLayout title='🔄 Retornos Médicos'>
      ...
      <ModalAgendarRetorno
        ...
        onAgendado={() => { setModal(null); carregar({ status: filtroStatus, busca }) }}
      ...
      <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', marginBottom: '22px' }}>
        {Object.entries(STATUS_CFG).map(([key, cfg]) => (
          ...
          >
            <div style={{ fontSize: '1.6rem', fontWeight: 800, color: cfg.color }}>{contagem[key] ?? 0}</div>
            <div style={{ fontSize: '0.72rem', color: '#94a3b8', marginTop: '3px' }}>{cfg.emoji} {cfg.label}</div>
          </div>
        ))}
        <div style={{
          flex: '1 1 130px', padding: '14px 18px',
          ...
          <div style={{ fontSize: '1.6rem', fontWeight: 800, color: '#94a3b8' }}>{retornos.length}</div>
          <div style={{ fontSize: '0.72rem', color: '#94a3b8', marginTop: '3px' }}>📋 Total</div>
        </div>
      </div>
      ...
      {!loading && (
        <div className='table-wrapper'>
          <table className='data-table'>
            <thead>
              <tr>
                <th>Paciente</th><th>Médico</th><th>Atendimento</th>
                <th>Retorno em</th><th>Data Prevista</th><th>Prazo</th>
            ...
            <tbody>
              {retornos.length === 0 && (
                <tr><td colSpan={8} style={{ textAlign: 'center', color: '#64748b', padding: '2rem' }}>
                  {filtroStatus || busca ? 'Nenhum retorno com os filtros aplicados.' : 'Nenhum paciente com retorno pendente. 🎉'}
                </td></tr>
              )}
              {retornos.map(r => {
                ...
                return (
                  <tr key={r.prontuario_id} style={{ background: r.status === 'atrasado' ? 'rgba(239,68,68,0.04)' : undefined }}>
                    <td>
                      ...
                      {r.paciente_tel && <div style={{ fontSize: '0.72rem', color: '#64748b' }}>{r.paciente_tel}</div>}
                    </td>
                    <td>
